<div>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div
            class="relative block h-full rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
            <div class="grid gap-y-2">
                <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Online Users</div>
                <div class="text-3xl font-semibold tracking-tight text-gray-950 dark:text-white">{{ $onlineUsers }}
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="grid gap-y-2">
                ...
                <div class="text-3xl font-semibold tracking-tight text-gray-950 dark:text-white">{{ $dataConsumed }}
                </div>
            </div>
        </div>
        ...
        {{-- <div
            ...
            <div class="grid gap-y-2">
                ...
                <div class="text-3xl font-semibold tracking-tight text-gray-950 dark:text-white">{{ $currentRouter ===
                    'all' ? 'All Locations' : $currentRouter }}</div>
            </div>
        </div> --}}
    </div>
                        ...
                        @foreach($recentSessions as $session)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                                    {{ $session->acctstarttime ? \Carbon\Carbon::parse($session->acctstarttime)->format('M d, H:i') : '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                                    {{ $session->acctsessiontime ? gmdate('H:i:s', $session->acctsessiontime) : '-' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
