...
class TestNasPlanFiltering extends Command
{
     ...
     * @var string
     */
    protected $signature = 'test:nas-plan-filtering {router?} {router_id?}';
     ...
     * @var string
     */
    protected $description = 'Test router identifier based plan filtering';
    ...
    {
        $routerIdentifier = $this->argument('router') ?? 'router-001';
        $routerId = $this->argument('router_id') ?? 1;

        $this->info("Testing router plan filtering for Router: {$routerIdentifier}, Router ID: {$routerId}");

        $planFilterService = new PlanFilterService();

        // Check if router has custom assignments
        $hasCustom = $planFilterService->hasCustomAssignments($routerIdentifier, $routerId);
